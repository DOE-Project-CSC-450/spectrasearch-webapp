...
export default class Login extends Component {  
    ...
    handleSignUpSubmit = () =>{
        console.log("did we make it all the way here without errors")
        console.log("this is their first try at email", this.state.signUpEmail)
        console.log("their second try", this.state.signUpEmail2)
        console.log("this their password", this.state.signUpPassword)
        console.log("LOOOOOOK", this.state.signUpEmail === this.state.signUpEmail2)
        if (!(this.state.signUpEmail === this.state.signUpEmail2)){
            //modal error emails do not match, please check emails
            this.setState({open: true })
        }
        else{
            console.log("you did great babe")
          ...
          for (D = 0; D < this.state.serverData.length; D++){
            ...
            if (this.state.serverData[D].Username === this.state.username){
                if (this.state.serverData[D].Password === this.state.password){
                    ...
                    console.log(this.state.finalUserName);
